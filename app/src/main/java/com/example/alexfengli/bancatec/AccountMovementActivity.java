...
public class AccountMovementActivity extends AppCompatActivity {
        ...
        service.getService().getAccountMovement(accountId, new Callback<List<MovimientoCuentaDTO>>() {
            ...
            public void success(List<MovimientoCuentaDTO> movimientoCuentaDTOs, Response response) {
                ...
                for(int i = 0;i<movimientoCuentaDTOs.size();i++)
                {
                    ...
                    temp.put("first",movimientoCuentaDTOs.get(i).getDetalleMovimiento().getFecha().toString());
                    int operacion = movimientoCuentaDTOs.get(i).getTipo();
                    switch (operacion) {
                        case 0:
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            temp.put("second","Compra");
                            break;
                    }
                    temp.put("third",movimientoCuentaDTOs.get(i).getDetalleMovimiento().getDescripcion());
                    temp.put("fourth",String.valueOf(movimientoCuentaDTOs.get(i).getDetalleMovimiento().getMonto()));
                    movements.add(temp);
                }

                setListView(movements);
            }
    ...
    private void setListView(ArrayList<HashMap<String,String>> movements)
    {
        ListView lv = (ListView)findViewById(R.id.AccountMoveListView);
        AccountMovementAdapter adapter = new AccountMovementAdapter(this,movements);
